...
export async function checkSeatAvailability(trainNumber, date, classType) {
  // Your code here
  if(typeof(trainNumber) !== "string" || trainNumber.length !== 5 || isNaN(trainNumber))
  ...
  let available = Math.random() < 0.7 ? true : false;

  seats === 0 ? available = false : available;

  let waitlist = Math.floor(Math.random()*20);
...
export async function bookMultipleTickets(passengers, trainNumber, date, classType) {
  ...
  for(let i=0;i<passengers.length;i++){
    ...
    try{
      ...
      const res = await bookTicket(passengers[i], trainNumber, date, classType)

      results.push(res)

    }catch(error){

      results.push({ passenger: passengers[i], error: error.message })
...
export async function raceBooking(trainNumbers, passenger, date, classType) {
  ...
  const promises = trainNumbers.map(train => bookTicket(passenger, train, date, classType).then(res => {
