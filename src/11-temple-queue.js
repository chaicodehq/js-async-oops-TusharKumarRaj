...
export class TempleQueue {
  ...
  enqueue(name, type) {
    ...
    if(!name || (type !== "regular" && type !== "vip") || this.#devotees.length >= this.#maxCapacity)
      return null
  ...
  static merge(queue1, queue2) {
    ...
    const newQueue = new TempleQueue(`${queue1.templeName}-${queue2.templeName}`, queue1.toArray().length + queue2.toArray().length)

    const arr1 = queue1.toArray()

    const arr2 = queue2.toArray()

    arr1.forEach(d => newQueue.#devotees.push(d))

    arr2.forEach(d => newQueue.#devotees.push(d))
